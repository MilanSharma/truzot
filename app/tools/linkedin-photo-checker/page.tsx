  ...
  const analyze = () => {
    ...
    img.onload = () => {
      ...
      setTimeout(() => {
        setResult({
          total,
          brightness: { score: Math.round(brightScore), text: brightScore > 80 ? "Perfect lighting" : brightScore > 50 ? "A bit too dark/bright" : "Poor lighting detected" },
          colorCast: { score: Math.round(colorScore), text: colorScore > 80 ? "Natural colors" : "Color tint detected" },
          complexity: { score: Math.round(complexScore), text: complexScore > 80 ? "Clean background" : "Background might be too distracting" }
        });
        setAnalyzing(false);
      }, 1500);
    };
    ...
    <div className="min-h-screen bg-[var(--bg)] text-[var(--text)]">
      ...
      <main className="pt-32 pb-24 px-6 max-w-4xl mx-auto">
        <div className="text-center mb-12">
          <p className="text-[var(--lime)] font-bold uppercase tracking-widest text-xs mb-4">Free Tool</p>
          <h1 className="text-4xl md:text-5xl font-black mb-4">LinkedIn Photo Analyzer</h1>
          <p className="text-[var(--text-muted)] text-lg">Upload your current profile picture for an instant AI analysis of lighting, background, and color cast.</p>
        </div>
        ...
        <div className="bg-[var(--surface)] border border-[var(--border)] rounded-3xl p-8 shadow-[var(--shadow-lg)]">
          {!image ? (
            <div className="border-2 border-dashed border-[var(--border)] rounded-2xl p-16 text-center hover:border-[var(--lime-border)] transition-colors relative">
              ...
              <p className="text-[var(--text-faint)]">JPG or PNG. We do not store this image on our servers.</p>
            </div>
          ) : (
            <div className="grid md:grid-cols-2 gap-12 items-center">
              <div className="text-center">
                ...
                <button onClick={() => {setImage(null); setResult(null);}} className="text-sm font-bold text-[var(--text-muted)] hover:text-[var(--text)] transition">Upload different photo</button>
              </div>
                ...
                {!result ? (
                  <div className="text-center">
                    {analyzing ? (
                      ...
                      <button onClick={analyze} className="btn-primary w-full text-lg py-4">Analyze Photo</button>
                    )}
                  </div>
                ) : (
                  <div className="space-y-6 animate-in slide-in-from-right-8">
                    <div className="flex items-center justify-between border-b border-[var(--border)] pb-6">
                      ...
                      <div className={`text-4xl font-black ${result.total > 80 ? 'text-[var(--success)]' : result.total > 50 ? 'text-[var(--warning)]' : 'text-[var(--error)]'}`}>
                        {result.total}/100
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      <div className="flex justify-between items-center">
                        <div>
                          ...
                          <div className="text-sm text-[var(--text-muted)]">{result.brightness.text}</div>
                        </div>
                        <div className="font-bold">{result.brightness.score}</div>
                      </div>
                      <div className="flex justify-between items-center">
                        <div>
                          <div className="font-bold">Color Balance</div>
                          <div className="text-sm text-[var(--text-muted)]">{result.colorCast.text}</div>
                        </div>
                        <div className="font-bold">{result.colorCast.score}</div>
                      </div>
                      <div className="flex justify-between items-center">
                        <div>
                          <div className="font-bold">Background Cleanliness</div>
                          <div className="text-sm text-[var(--text-muted)]">{result.complexity.text}</div>
                        </div>
                        <div className="font-bold">{result.complexity.score}</div>
                      </div>
                    </div>

                    <div className="pt-6 border-t border-[var(--border)]">
                      <Link href="/upload" className="btn-primary w-full flex justify-center text-lg py-4">
