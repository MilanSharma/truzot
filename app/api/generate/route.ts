...
async function enqueueNextBatch(orderId: string): Promise<boolean> {
 ...
 if (!qstashToken) {
 log.error(
 { orderId, env: process.env.NODE_ENV },
 "QSTASH_TOKEN missing - using inline fallback. This is NOT recommended for production as it may cause stuck orders due to Vercel timeout limits. Please set QSTASH_TOKEN environment variable."
 );
 waitUntil(
...
export const POST = withContext(async (req: Request) => {
 ...
 .update({ 
 status: "completed",
 preferences: { ...prefs, generate_failures: 0 }
 })
 .eq("id", orderId)
 .eq("status", "generating");
 await supabaseAdmin
