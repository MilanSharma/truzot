...
export async function POST(req: Request) {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    if (!orderId) return NextResponse.json({ error: 'Missing metadata' }, { status: 400 });
    const { data: order, error: fetchError } = await supabaseAdmin.from('orders').select('zip_url, status').eq('id', orderId).single();
    if (fetchError || !order?.zip_url) return NextResponse.json({ error: 'Order not found' }, { status: 400 });
    if (['training', 'generating', 'completed'].includes(order.status)) return NextResponse.json({ received: true });
    ...
    if (freshZipUrl.includes('/object/sign/')) {
      try {
        ...
        if (pathParts.length > 1) {
          ...
          const { data: newData } = await supabaseAdmin.storage.from('uploads').createSignedUrl(storagePath, 7200);
      ...
      } catch (e) { console.error('Failed to refresh zip URL:', e); }
    }
    await supabaseAdmin.from('orders').update({ status: 'training', stripe_payment_intent: session.payment_intent as string }).eq('id', orderId);
    await supabaseAdmin.from('trainings').upsert({ order_id: orderId, status: 'training' }, { onConflict: 'order_id' });
    try {
