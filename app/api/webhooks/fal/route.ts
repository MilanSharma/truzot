...
export const POST = withContext(async (req: Request) => {
 ...
 const retryCount = (prefs.retry_count as number) || 0;
      const trainingAttempts = (prefs.training_attempts as number) || 0;

 if (trainingAttempts < 3 && isValidTransition(currentStatus, "training")) {
 ...
 status: "training",
 zip_url: zipUrl,
 preferences: { ...prefs, training_attempts: trainingAttempts + 1 },
 })
 .eq("id", orderId);
 if (orderErr)
 ...
 await stripe.refunds.create({
            ...
            idempotencyKey: `fal-webhook-fail-refund-${orderId}`
          });
 await supabaseAdmin
 .from("orders")
 ...
 await supabaseAdmin.from("webhook_events").update({ status: "processed" }).eq("event_id", falEventId);
 waitUntil(
